  ...
  function setRemoteDescription(description, onSuccess, onFailure)
{
  ...
  var parameters = this._parseRemoteDescription(description)[0];
  this._iceTransport.start(this._iceGatherer, parameters.iceParameters, 'controlling');
  this._dtlsTransport.start(parameters.dtlsParameters);
  ...
  function _parseRemoteDescription(description)
{
  ...
  // We are going to check session- and media-levels for ICE and DTLS
  // parameters.
  var levels = [sdp];
  if (sdp.media) {
