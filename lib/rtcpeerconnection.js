...
RTCPeerConnection.prototype.createAnswer = function createAnswer(onSuccess, onFailure) {
  ...
  var kinds = getTrackKinds(streams);
  var mids = new Set();
  ...
  remote.media.forEach(function(media) {
    ...
    var sendrecv = media.direction === 'sendrecv';
    var sendonly = media.direction === 'sendonly';
    var recvonly = media.direction === 'recvonly';
    var inactive = media.direction === 'inactive';

    var capabilities;
    var direction;

    if (inactive || (recvonly && !send) || !sharedCodecs.length) {
    ...
    sdpUtils.addMediaToSDPBlob(sdp, kind, capabilities,
      sendonly ? null : streams, direction, mid, 'active');
  });

  sdpUtils.addBundleToSDPBlob(sdp, mids);
  // TODO(mroberts): Filter MediaStream IDs to those included in the answer.
...
RTCPeerConnection.prototype.createOffer = function createOffer(onSuccess, onFailure, options) {
  ...
  var receive = new Map();
  [options.optional, options.mandatory].forEach(function(options) {
    options = options || {};
    if ('OfferToReceiveAudio' in options) {
      receive.set('audio', options.OfferToReceiveAudio);
    }
    if ('OfferToReceiveVideo' in options) {
      receive.set('video', options.OfferToReceiveVideo);
    }
  });
  ...
  // TODO(mroberts): Instead of iterating over the kinds, why not iterate over
  // the MediaStreams?
  ['audio', 'video'].forEach(function(kind) {
    var send = kinds.has(kind);
    var recv = receive.get(kind);
    ...
    // TODO(mroberts): Right now, there is a one-to-one mapping between kind
    // and mid. That should change.
    var mid = kind;
    mids.add(mid);

    sdpUtils.addMediaToSDPBlob(sdp, kind, capabilities,
      streams, direction, mid);
  });
...
/**
 * Get the set of kinds represented by the MediaStreamTracks in an Array of
 * MediaStreams.
 * @param {?Array<MediaStream>} [streams]
 * @returns {Set<string>}
 */
function getTrackKinds(streams) {
  var hasAudio = false;
  var hasVideo = false;
  if (streams) {
    for (var i = 0; i < streams.length; i++) {
      hasAudio = hasAudio || streams[i].getAudioTracks().length;
      hasVideo = hasVideo || streams[i].getVideoTracks().length;
